var EventManagementPage = (function(){
  ...
  function bindLists(){ function handle(e){ var t=e.target.closest('a[data-id]'); if(!t) return; e.preventDefault(); var id=t.getAttribute('data-id'); if(!id) return; loadDetail(id); } var up=document.getElementById('upcoming-list'); var pa=document.getElementById('past-list'); var sr=document.getElementById('search-results'); if(up) up.addEventListener('click', handle); if(pa) pa.addEventListener('click', handle); if(sr) sr.addEventListener('click', handle); }
